# 📱 Your Flutter Journey

## #1 Beginner Level

**Build your technical foundation.**

This is where you start building your foundation in Flutter. You'll learn Dart basics & OOP, explore widgets, create simple screens.

## #2 Intermediate Level

**Expand your skills and take on real challenges.**

Here you'll dive deeper into Flutter development. You'll learn handle navigation, and build forms. You'll also learn how to fetch data from the internet and store it locally. By the end of this level, you'll complete a mini project to put everything together.

## #3 Advanced Level

**Master Flutter and build production-ready apps.**

Here you'll dive deeper into Flutter development. You'll work with async/await and streams, manage state using Cubit/Bloc, and understand app architecture patterns like MVVM. By the end of this level, you'll complete a medium-sized project with advanced features.

---

## Flutter Journey Phases

### Beginner Level

- Week 1
- Week 2
- Week 3
- Week 4
- Week 5
- Week 6
- Week 7
- Final Project

### Intermediate Level

- Week 8
- Week 9
- Week 10
- Week 11
- Week 12
- Week 13
- Week 14
- Week 15
- Final Project

### Advanced Level

*Coming Soon! .*

---

## Flutter Journey Steps (Detailed)

### #1 Beginner Level (Detailed)

| Week              | Tags    |
| :---------------- | :------ |
| **Week 1**        | Concept |
| **Week 2**        | Coding  |
| **Week 3**        | Coding  |
| **Week 4**        | Coding  |
| **Week 5**        |         |
| **Week 6**        | Project |
| **Week 7**        | Coding  |
| **Final Project** |         |

### #2 Intermediate Level (Detailed)

| Week / Step       | Tags    |
| :---------------- | :------ |
| **Week 8**        | Concept |
| **Week 9**        | Coding  |
| **Week 10**       |         |
| **Week 11**       |         |
| **Week 12**       | Coding  |
| **Week 13**       | Coding  |
| **Week 14**       |         |
| **Week 15**       |         |
| **Final Project** | Project |

---

> **Note:** Each week listed in [Level 1](./Level-1/) / [Level 2](./Level-2/) Folders and leading to detailed learning guides, tool recommendations, and resources.
